...
public class ConfigStoreTests
{
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), $"atomdpi-cfg-{Guid.NewGuid():N}");
    ...
    public void DefaultsAreUsableWhenNothingHasBeenSavedYet()
    {
        ...
        Assert.Equal(AtomDpi.Core.Dns.DnsMode.EncryptedLoopback, settings.DnsMode);
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"atomdpi-bad-{Guid.NewGuid():N}.json");
...
public class ProtectionServiceDescriptionTests
{
    ...
    public void ScopeDescriptionsAreFilledInForEveryValue()
    {
        ...
        {
            Assert.False(string.IsNullOrWhiteSpace(AtomDpi.Core.ProtectionService.DescribeScope(scope)));
        }
    }
    ...
    {
        foreach (var outcome in Enum.GetValues<AtomDpi.Core.Diagnostics.ProbeOutcome>())
        {
            Assert.False(string.IsNullOrWhiteSpace(AtomDpi.Core.ProtectionService.DescribeOutcome(outcome)));
        }
    }
}

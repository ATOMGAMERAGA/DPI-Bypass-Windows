...
public class DnsConfiguratorTests
{
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), $"dpibypass-test-{Guid.NewGuid():N}");
...
public class FakePayloadTests
{
    ...
    {
        Assert.NotEmpty(DpiBypass.Core.Engine.FakePayloadFactory.DecoyHosts);

        foreach (var host in DpiBypass.Core.Engine.FakePayloadFactory.DecoyHosts)
        {
    ...
    {
        var request = DpiBypass.Core.Engine.FakePayloadFactory.CreateHttpRequest("www.microsoft.com");
    ...
    {
        var request = DpiBypass.Core.Engine.FakePayloadFactory.CreateHttpRequest("www.bing.com", minimumLength: 400);
    ...
    {
        var first = DpiBypass.Core.Engine.FakePayloadFactory.CreateTlsClientHello("www.bing.com");
        var second = DpiBypass.Core.Engine.FakePayloadFactory.CreateTlsClientHello("www.bing.com");

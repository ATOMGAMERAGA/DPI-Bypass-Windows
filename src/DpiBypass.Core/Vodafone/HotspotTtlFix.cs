...
/// <summary>
/// The TTL rewrite behind Vodafone Sınırsız Modu, as something that can be swapped out.
/// </summary>
/// <remarks>
/// An interface because <see cref="HotspotTtlFix"/> needs the WinDivert driver and
/// administrator rights, and the service's decision about <i>when</i> to install the rule
/// is worth testing on its own - on a build machine that has neither.
/// </remarks>
public interface IHotspotTtlFix : IDisposable
{
    /// <summary>Whether a rule is installed right now.</summary>
    bool IsActive { get; }

    /// <summary>The adapter the rule is scoped to, or 0.</summary>
    int InterfaceIndex { get; }

    TtlFixSettings Settings { get; }

    /// <summary>Packets whose TTL was actually rewritten.</summary>
    long RewrittenPackets { get; }
...
/// default) means the phone's decrement leaves exactly 64 on the wire. This is the
/// Windows counterpart of the Linux build's <c>dpibypass/vodafone.py</c>, which does
/// the same arithmetic in an nftables postrouting chain.
/// </para>
/// <para>
...
public sealed class HotspotTtlFix : IHotspotTtlFix
{
    ...
    /// </summary>
    /// <remarks>
    /// Rewriting the TTL invalidates the IPv4 header checksum, and a packet whose
    /// checksum was not recomputed is one the next hop discards - so a failure here would
    /// stall the connection rather than merely leave it recognisable. Those packets go
    /// out unmodified instead, which costs the feature and not the link.
    /// </remarks>
    public long ChecksumFailures => Interlocked.Read(ref _checksumFailures);
    ...
    public void Apply(int interfaceIndex, TtlFixSettings settings)
    {
        ...
        lock (_gate)
        {
            ...
            {
                // The driver payload is fetched by the build pipeline rather than carried
                // in the repository, so a developer build - and a damaged install - reach
                // this instead of a Win32 error. It is the feature failing to start, not
                // the process failing, so it leaves as the same exception as any other
                // refusal to install the rule.
                throw new TtlFixException(
                    "WinDivert sürücüsü yüklenemedi; kurulum klasöründeki WinDivert dosyalarını denetleyin.",
                    ex);
            }
        ...
        try
        {
            while (!stopping.IsCancellationRequested)
            {
                ...
                if (change is { } rewrite)
                {
                    ...
                    {
                        // The IPv4 header checksum covers the TTL, so a packet we changed
                        // and could not re-checksum is one the next hop drops. Putting it
                        // back the way it arrived costs the rewrite for this packet and
                        // keeps the connection working, which is the right way round.
                        packet[rewrite.Offset] = rewrite.Previous;
                        Interlocked.Increment(ref _checksumFailures);

                        if (Interlocked.Exchange(ref _checksumFailureLogged, 1) == 0)
                        {
    ...
    /// <summary>
    /// The guard, as a pure function, so the invariant can be tested without a driver.
    /// </summary>
    /// <returns>Where the TTL was written and what was there, or null if untouched.</returns>
    internal static (int Offset, byte Previous)? Rewrite(
        Span<byte> packet,
        int offset,
        TtlFixSettings settings)
    {

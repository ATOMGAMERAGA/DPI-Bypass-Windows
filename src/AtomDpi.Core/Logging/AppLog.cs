...
namespace AtomDpi.Core.Logging;
...
public static class AppLog
{
    ...
    public static void Initialise()
    {
        try
        {
            ...
            _filePath = Path.Combine(AppPaths.LogDirectory, $"atomdpi-{DateTime.Now:yyyy-MM-dd}.log");
            PruneOldFiles();
        }
        catch (Exception)
        {
    ...
    private static void PruneOldFiles()
    {
        try
        {
            ...
            foreach (var file in Directory.EnumerateFiles(AppPaths.LogDirectory, "atomdpi-*.log"))
            {
